...
void Heap<Type>::push(const Type& v)
{
	...
	{
		nodePtr = lastNodeInsertedPtr;
		while ((nodePtr->parentPtr && (nodePtr->parentPtr->rightChildPtr == nodePtr)) || !(!nodePtr->parentPtr || (nodePtr = nodePtr->parentPtr->rightChildPtr)))
		{
			nodePtr = nodePtr->parentPtr;
		}
		...
		!nodePtr->parentPtr->rightChildPtr && (nodePtr = nodePtr->parentPtr);
		nodePtr->leftChildPtr ? (nodePtr->rightChildPtr = newNode) : (nodePtr->leftChildPtr = newNode);
